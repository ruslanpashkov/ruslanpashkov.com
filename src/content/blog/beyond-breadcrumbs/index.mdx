...
There's also an interesting point about lists and separators. According to the [W3C specification](https://html.spec.whatwg.org/multipage/grouping-content.html#the-ol-element), lists can **only** contain `<li>` elements _(actually, also `<script>` and `<template>`, but that's a topic for another day)_. This tells us that our separators must be inside the list items, not between them, even though, from a purely academic standpoint, a separator isn't a content part of the list item. Also, worth noting that the **last list item shouldn't contain a separator**.

The most attentive might have noticed that in the W3C specification mention, I linked specifically to the ordered list. Why that?
...
<Image
	...
	alt="Screenshot of Google Search Central breadcrumbs showing 'Home > Search Central > Documentation' with DevTools inspector revealing unordered list (ul) implementation"
	sizes={`(max-width: 768px) 100vw, ${googleSearchCentralBreadcrumbs.width}px`}
	widths={[400, 768, 1024]}
	decoding="async"
	loading="lazy"
	quality="max"
...
<nav>
	<ol>
		...
		<li>
			<a href="/new-buildings/green-quarter">Green Quarter Complex</a>
		</li>
	</ol>
</nav>
```
...
First, we should label our navigation—it's not enough to just mark it as a navigation section. As we could hear in our example, the screen reader didn't announce this as navigation. Since our page might have multiple navigation elements, we want screen readers to know exactly what kind of navigation this is:
...
And for the final touch—we want to tell users which page they're currently on. Just like we highlight the current link visually for regular users, we want screen readers to announce it for people with disabilities. That's what `aria-current="page"` is for:

```html ins="aria-current="page""
<li>
	<a href="/new-buildings/green-quarter" aria-current="page"> Green Quarter Complex </a>
</li>
```
...
<nav aria-label="Breadcrumbs">
	<ol itemscope itemtype="https://schema.org/BreadcrumbList">
		...
		<li itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem">
			<a itemprop="item" href="/new-buildings/green-quarter" aria-current="page">
...
<Image
	...
	sizes={`(max-width: 768px) 100vw, ${googleBreadcrumbsExample.width}px`}
	widths={[400, 768, 1024]}
	decoding="async"
	loading="lazy"
	quality="max"
...
<nav class="breadcrumbs" aria-label="Breadcrumbs">
	<ol class="breadcrumbs-list">
		...
		<li class="breadcrumbs-item">
			<a class="breadcrumbs-link" href="/new-buildings">
				New Buildings
			</a>
			<span class="breadcrumbs-separator" aria-hidden="true">»</span>
		</li>
		<li class="breadcrumbs-item">
			<a href="/new-buildings/green-quarter" aria-current="page">
			<a class="breadcrumbs-link" href="/new-buildings/green-quarter">
		...
		<li class="breadcrumbs-item">
			<a
				...
				href="/new-buildings/green-quarter/gallery"
				aria-current="page"
			>
				Gallery
			</a>
...
Also, it would be better to extend the clickable area of the links to make them easier to interact with:
...
<Image
	...
	sizes={`(max-width: 768px) 100vw, ${styledBreadcrumbsFoundation.width}px`}
	widths={[400, 768, 1024]}
	decoding="async"
	loading="lazy"
	quality="max"
...
By the way, we should make the current page link inaccessible for mouse and keyboard users while keeping it readable for screen readers. We can do this by adding `tabindex="-1"` and `pointer-events: none` to the link with `aria-current="page"`:
...
```html ins={9-20} ins=/hidden(?=>)/ {"Add hidden attribute to items between first and last two":21-22}
<nav class="breadcrumbs" aria-label="Breadcrumbs">
	<ol class="breadcrumbs-list">
		<li class="breadcrumbs-item">
		...
		<li class="breadcrumbs-item" hidden>
			<a class="breadcrumbs-link" href="/new-buildings"> New Buildings </a>
			<span class="breadcrumbs-separator" aria-hidden="true">»</span>
		</li>
		<li class="breadcrumbs-item">
			<a class="breadcrumbs-link" href="/new-buildings/green-quarter">
...
And the corresponding CSS to hide the items:
...
const initBreadcrumbsNavigation = () => {
	...
	const breadcrumbItems = breadcrumbs?.querySelectorAll('.breadcrumbs-item');
	const expandButton = breadcrumbs?.querySelector('#breadcrumbs-expand');

	if (!breadcrumbs || !breadcrumbItems || !expandButton) {
		return;
	}

	const showAllItems = () => {
		breadcrumbItems.forEach((crumb) => crumb.removeAttribute('hidden'));
...
<Image
	...
	sizes={`(max-width: 768px) 100vw, ${breadcrumbsExpandExample.width}px`}
	widths={[400, 768, 1024]}
	decoding="async"
	loading="lazy"
	quality="max"
...
<Image
	...
	sizes={`(max-width: 768px) 100vw, ${focusedBreadcrumbsPreview.width}px`}
	widths={[400, 768, 1024]}
	decoding="async"
	loading="lazy"
	quality="max"
...
const initBreadcrumbsScroll = () => {
	const breadcrumbs = document.querySelector('.breadcrumbs');
	const breadcrumbList = breadcrumbs?.querySelector('.breadcrumbs-list');

	if (!breadcrumbs || !breadcrumbList) {
		return;
	}

	const checkScroll = () => {
		const isAtStart = breadcrumbList.scrollLeft <= 1;
		const isAtEnd =
			breadcrumbList.scrollLeft + breadcrumbList.clientWidth >=
			breadcrumbList.scrollWidth - 1;

		breadcrumbs.classList.toggle('show-start-fade', !isAtStart);
		breadcrumbs.classList.toggle('show-end-fade', !isAtEnd);
	};

	breadcrumbList.addEventListener('scroll', checkScroll);
...
<Image
	...
	sizes={`(max-width: 768px) 100vw, ${mobileBreadcrumbsWithMask.width}px`}
	widths={[400, 768, 1024]}
	decoding="async"
	loading="lazy"
	quality="max"
...
```typescript title="utils/schema.ts"
...
export function BreadcrumbsClient({ crumbs }: Props) {
	...
	const [isExpanded, setIsExpanded] = useState(crumbs.length < MAX_VISIBLE_ITEMS);
	...
	useEffect(() => {
		const checkScroll = () => {
			...
			const isEndFadeVisible =
				list.scrollLeft + list.clientWidth < list.scrollWidth - SCROLL_THRESHOLD;
	...
	const visibleCrumbs = isExpanded
		? crumbs
		: [crumbs[0], { label: EXPANDER_LABEL, href: '#' }, ...crumbs.slice(-2)];
